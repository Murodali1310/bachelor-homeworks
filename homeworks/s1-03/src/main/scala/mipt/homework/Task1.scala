...
object Task1 extends App {
  ...
  def takeWhile[A](l: MyList[A], predicate: A => Boolean): MyList[A] = {
    @tailrec
    def takeWhileRec[B](l: MyList[B], predicate: B => Boolean, acc: MyList[B]): MyList[B] =
      l match {
        case MyNil => acc
        case MyCons(head, tail) => if (predicate(head)) takeWhileRec(tail, predicate, MyCons(head, acc)) else acc
      }
  ...
  def dropWhile[A](l: MyList[A], predicate: A => Boolean): MyList[A] = l match {
    case MyNil => MyNil
    case MyCons(head, tail) => if (predicate(head)) dropWhile(tail, predicate) else l
  }
  /**
  ...
  def zip[A, B](l1: MyList[A], l2: MyList[B]): MyList[(A, B)] = {
    @tailrec
    def zipRec[C, D](l1: MyList[C], l2: MyList[D], acc: MyList[(C, D)]): MyList[(C, D)] =
      l1 match {
        case MyCons(head1, tail1) => l2 match {
          case MyCons(head2, tail2) => zipRec(tail1, tail2, MyCons((head1, head2), acc))
          case MyNil => acc
        }
        case MyNil => MyNil
      }
